...
### 0-28. v3.8.8 → v3.8.9 (D151의 "닫힘"을 보충한다 — 부분 닫힘과 결정에 의한 분리, 2026-09-17)
**어디서 나왔나.** `g0-review.py --count`가 첫 수를 냈다(PR #25·#26) — 범위 안 `dedupe_key` 총 21, 닫힘 0, 열림 21. 닫힘 표시가 기록에 하나도 없어서다: 발견 1이 D146–D149로, D137의 근거가 D150으로 닫힌 것은 `prd.md`에 있지만 기록에는 안 적혔고, 계산기가 `prd.md`를 읽어 추측하면 그 수가 다시 사람 보고가 된다. 닫힘 표시를 기록에 넣는 판을 앞두고 구현 세션이 물었다: 한 key가 여러 D에 걸쳐 닫히거나, **D가 key의 일부만 닫는 경우** 어떻게 세는가. 계약에 없었다.

**결정(D153).** 여럿으로 닫히는 것은 문제가 아니다 — `닫힘 — D146·D147·D148`처럼 적고 계산기는 표시의 유무만 본다. 일부만 닫힌 경우에는 **"부분 닫힘"이라는 셋째 상태를 만들지 않는다.** D151의 단위는 (절 집합, 주장 하나)인데 D가 일부만 닫았다면 그 "주장 하나"가 실은 둘이었다는 뜻이고, 그것은 dedupe가 덜 된 것이다. 그러니 key를 **쪼갠다** — 닫힌 부분은 새 key로 떼어 `닫힘 — D<n>`, 남은 부분은 원래 key로 열린 채. 상태를 늘리면 `--count`가 "열림/닫힘/부분"을 세게 되고 "부분"이 곧 판단이 되어 계산이 사람 보고로 돌아간다.

**쪼갬의 주체.** 종합자의 쪼갬은 막아야 한다(PR #24가 실제로 잡았다 — 발견 15는 R1-7의 일부를 종합자가 떼어 세운 것이고 계산기가 한 key로 접는다, 반례 05). 결정의 분리는 허용해야 한다. 둘을 계산기가 가르는 유일한 표지가 **D 번호**다 — 대응표의 "결정에 의한 분리" 행에는 D 번호가 반드시 붙고, D 번호 없는 분리는 쪼갬으로 접는다(계산기 반례 09 "D 번호가 붙은 분리를 쪼갬으로 접지 않는가"가 여기 붙는다 — 별도 판). 그리고 닫힘의 **귀속은 주장으로** 한다, 절 겹침으로 하지 않는다 — 1차 key 12(§2.7↔§5.5, 머지 제외의 집합)와 D150(§2.7, 90일 절단)이 §2.7을 공유하지만 주장이 다르다; 절로 매기면 D150이 key 12를 닫은 것으로 오귀속된다.
...
### 0-30. v3.9.0 → v3.9.1 (저작 군 — 교차 검토 key 셋을 한 결정 묶음으로, 2026-09-22)
**둘째 군.** 재현성 군(§0-29)과 같은 모양이다 — 세 key가 한 질문의 세 면이었다: *`people.attribution` 모드가 산출물의 무엇을 바꾸고, 그것이 id에 어떻게 들어가는가.* 기록(`docs/review/records/`, 개별이 정본)의 주장으로:
| key | 절 | 주장 | 어느 면 |
|---|---|---|---|
| S2 (R1-10·R2-9·R3-2·R5-8·O-8, 전원 모순) | §2.1·§2.7·§2.8·§2.9·§4.3 | 모드는 산출물을 바꾸는데 §2.8 id 입력·§7 `reproduce`·스키마 어디에도 없다 — 같은 id로 off 결과와 team 결과가 서로 캐시 적중한다 | 입력 |
| S9 (R1-11·R2-7·R2-8·R3-7) | §2.1·§2.7·§2.9 | `individual`이 낼 저자 사실이 갈 스키마가 7종 안에 없고(`report`는 `author`를 조건 없이 거부), "≥3인 집계"가 무엇에 거는 조건인지 없다 | 자리와 k |
| R4-6 | §2.1·§2.7 | off 모드가 내는 것의 목록이 §2.1 표와 §2.7 문장에서 같지 않다("등"으로 열림) — 기여자 1명인 파일이 `distinct_authors_90d = 1`로 나가는 것이 재식별인가 | 목록 |

**정한 것(D160–D162).** (1) **입력** — `people.attribution`과 `team_mapping_sha256`은 id 입력이고 `reproduce.people`로 인쇄한다(`parameters` 안이 아닌 이유는 B.6의 구분이 인쇄 자리에도 남아야 하기 때문; id 입력이라는 사실은 `[id]`가 말한다). 매핑 해시는 정체가 아니다 — 저자 해시와 달리 파일 하나의 해시. 사영은 `team_count`와 저자 사실에만(D159). §4.3의 "정책이지 파라미터가 아니다"는 그대로 참이되 재현성 면제로 읽히지 않게 고쳤다. (2) **자리와 k** — 저자 사실은 8번째 산출물 `people.json`; `report`의 하드룰은 조건 없이 그대로; k 하한은 팀 집계의 조건이라 3인 미만 팀은 `other`로 접고 `team_count`·`teams_folded`로 낸다 — 2인 팀 하나만 만진 파일은 `1, 1`이다: 값은 있고 정체는 없다(null도 억제도 아니다). (3) **목록** — off가 내는 저자 유래 필드는 셋으로 닫히고 `team_count`는 필드 부재; k는 익명 집계에 걸지 않는다 — `distinct_authors_90d = 1`은 사실이지 정체가 아니며 재식별에 필요한 `git log`가 저자를 직접 보여 주니 이 필드가 더하는 것이 없다(D108의 논거).

**포기한 것.** `report`가 `attribution`에 따라 `author`를 조건부 허용하는 안 — D126의 변조 전체가 "조건 없을 때"를 겨눠야 해 방어가 약해진다. 저자 사실을 MCP 응답에만 두는 안 — 켠 사람이 자기 산출물을 볼 수 없다. `team_count`를 k 미달에서 `null`로 두는 안 — "작은 팀 하나가 만졌다"는 사실까지 지운다; 접으면 사실은 남고 라벨만 사라진다. k를 익명 집계에도 거는 안 — `distinct_authors_90d`를 1에서 `null`로 만들면 유용한 사실을 지우면서 재식별을 막지 못한다(막는 것은 이미 `git log`에 있다).

**이 판이 닫는 key(이름만 — 닫힘 표시는 후속 PR).** S2(D160), S9(D161), R4-6(D162). 셋 전부, 부분 없음. 귀속은 주장으로 했다(D153).

**이 판 뒤 스키마가 따라가야 할 자리.** `schemas/people.schema.json`(8번째 — `contract/schema/`·CLAUDE.md·`docs/status.md`의 "7종" 문장이 "8종"으로 따라간다); `report.schema.json`의 `reproduce.people`(required: `attribution`·`team_mapping_sha256`, off면 후자 `null`)과 최상위 `people_artifact`(`"people.json" | null`); `contract/schema/` 변조: "off인데 `people_artifact`가 `people.json`이다 → 거부", "`people.json`에 `.jqradar/` 경로가 있다 → 거부", "`reproduce.people` 없음 → 거부(D111)", "`team_count`만 있고 `teams_folded` 없음 → 거부", "`report`의 파일에 `author` → 거부(D126 그대로)"; `contract/reproducibility/`: attribution 토글 → id 변경 ∧ `repository_state_id` 불변, 매핑 한 줄 수정 → `team_count` 키만 미스; `contract/history/`: 2인 팀 → `1, 1`, off 부재 검사(`must_not_be_caught` 포함). 이 판이 커밋되면 §7 예시가 `reproduce.people`과 `people_artifact`를 얻어 현재 스키마(`additionalProperties: false`)에 걸려 스위트가 붉다 — 계약이 먼저, 스키마가 같은 PR의 다음 커밋으로(PR #11·#17·#31의 순서).

**이 판이 하지 않은 것.** 닫힘 표시, 스키마·픽스처, 나머지 key. G0 #1은 통과가 아니다 — 열린 key가 남았고 라운드 조건(D151)도 아직이다.

### 0-31. v3.9.1 → v3.9.2 (`author_id`의 정의 — 한 줄 판, 2026-09-22)
**교차 검토 key가 아니다.** v3.9.1이 열어 둔 공백이다(§0-30 "이 판이 하지 않은 것" — `people.json`을 만들면서 `authors[].author_id`가 무엇인지(매핑 파일의 식별자? 이메일?)를 계약이 말하지 않았고, PR #33의 `people.schema.json`이 `x-jqradar-unresolved.author_id`로 표시했다). 렌즈 군보다 먼저 한 줄로 닫는다 — 쌍·순서 군(다음 판)에 섞으면 "한 결정 묶음 = 한 질문"이 깨진다.

**정한 것(D163).** `author_id`는 **조직 팀 매핑 파일이 정한 식별자**다 — git 이름·이메일이 아니다. jQRadar는 git 저자를 그대로 내지 않고 매핑이 준 키로만 낸다: D48의 "추론 금지"와 같은 선이고, `individual`을 켠 조직도 자기가 정한 식별자만 받는다. 매핑에 없는 저자는 `unmapped`로 접고 파일마다 `authors_unmapped: n`을 병기한다 — 팀의 `other`·`teams_folded`(D161)와 같은 처리: 값은 있고 정체는 없다. 식별자의 형식은 매핑 파일이 정하므로(조직의 것, B.6) 스키마는 `type: string`과 "매핑 파일의 키"라는 설명만 갖고, 형식 `pattern`은 조직이 매핑을 주면 그때 걸 수 있다.

**스키마가 따라갈 자리.** `people.schema.json` — `author_id`의 설명(매핑 파일의 키, git 식별자 아님), `fileFacts`에 `authors_unmapped`(integer ≥ 0; `individual`에서 `authors`와 함께 required, `team`에서는 `null`), `x-jqradar-unresolved.author_id` 삭제. `contract/schema/` 변조: "`author_id`가 이메일 모양(`@` 포함) → 거부"는 **걸지 않는다** — 형식은 조직의 것이라(B.6) 이메일을 식별자로 쓰는 조직을 계약이 막을 수 없다; 대신 설명에 "git 이메일을 그대로 넣는 것은 D163 위반이다 — 매핑이 그 이메일을 키로 정했을 때만 같은 모양이 정당하다"를 적는다. 이 판이 커밋되면 §7 `people.json` 블록이 `authors_unmapped`를 얻어 `people.schema.json`(`additionalProperties: false`)에 걸린다 — 붉은 자리는 `accept-people` 하나여야 한다.

**이 판이 하지 않은 것.** 닫힘 표시 없음(key가 아니다). 나머지 열린 key. G0 #1은 통과가 아니다.

### 0-32. v3.9.2 → v3.9.3 (쌍·순서 군 — 교차 검토 key 넷을 한 결정 묶음으로, 2026-09-22)
**셋째 군.** 네 key가 한 질문의 네 면이었다: *중복·공변경의 **단위**가 무엇인가.* 기록(개별이 정본)의 주장으로:
| key | 절 | 주장 | 어느 면 |
|---|---|---|---|
| S7 (일곱 전원 모순) | §2.1·§2.4·§2.7·§2.8 | §2.1의 이력 성분은 전부 "W 안"인데 `shared`만 없다 — 전 이력으로 읽으면 `tc > 1` | 공변경의 구간 |
| S4 (여덟) | §2.1·§2.6·§5.2·§5.3 | (a) §2.6의 발생은 3튜플, §5.2의 키는 2튜플 — 같은 파일 안 새 사본이 잡히지 않는다; (b) "발생 ≥ 2인 클러스터"는 거르는 것이 없고 자기 중복이 `new_duplication`에 들어와 게이트를 FAIL시킨다; 부수로 "클러스터 토큰 ≥ 100"은 기본에서 항상 참 | 새 중복의 단위 |
| R2-5 | §2.1·§2.6 | 혼합 클러스터(A 2회·B 1회)에서 §2.1과 §2.6이 `self_dup_tokens`에 다른 값을 낸다 | 자기 중복의 단위 |
| R1-9 | §2.9·§3.5·§3.7 | §3.5가 정렬 규칙을 둘 주면서 우선순위·tie-break가 없다 — 배열이라 순서가 바이트다 | 쌍의 순서 |

**정한 것(D164–D167).** (1) `shared`는 W 안, `tc ≤ 1`(D164) — 근거는 일관성이고 재현성이 아니다(D150). (2) 새 중복은 **파일×클러스터 수준의 새 발생 수**(`max(0, HEAD − BASE)`), 서로 다른 파일 ≥ 2인 클러스터만(D165) — 키에 오프셋을 넣지 않고(여덟 중 여섯의 의견) 같은 파일 안 사본은 개수로 센다; 게이트의 토큰 조건은 손잡이로 남기되 기본값이 `cpd.minimum_tokens`라 기본에서 거르지 않는다는 것을 문장으로 적었다(B.6 — 정책은 조직의 것). (3) `self_dup_tokens`는 한 파일에만 있는 클러스터의 것 — §2.6이 정본(D166), 혼합 클러스터의 값 넷(200·200·0·1)을 §2.6에 적었다. (4) 정렬 키(D167).

**포기한 것.** 키를 `(경로, 시작, 끝, 클러스터 ID)` 3튜플로(Sonnet 하나) — 오프셋이 무관한 편집에 흔들려 같은 사본이 새것으로 보인다. 같은 파일 안 사본을 세지 않는 안 — A↔B가 A·A'↔B가 되는 것은 중복이 늘어난 것이다. 게이트의 "≥ 100"을 지우는 안 — 손잡이는 조직의 것이고 기본값의 뜻을 적는 것으로 족하다. `self_dup_tokens`에 혼합 클러스터의 반복을 세는 안(§2.1 문자대로) — 그 토큰이 `pair_dup_tokens`에 이미 있어 두 번 셈이 된다.

**이 판이 닫는 key(이름만 — 닫힘 표시는 후속 PR).** S7(D164), S4(D165), R2-5(D166), R1-9(D167). 넷 전부, 부분 없음. S4와 R2-5는 다른 주장이라 각자 D다. 귀속은 주장으로(D153).

**이 판 뒤 픽스처가 따라가야 할 자리.** `contract/cpd/` 혼합 클러스터(값 200·200·0·1) · `contract/history/` `shared` 창 한정(`tc ≤ 1`)과 정렬 순서 · `contract/gate/`(G1)는 문장만 — 새 발생 수 넷과 `min_tokens < cpd.minimum_tokens` 프로필 거부 · `check` 프로필 스키마의 `gate.new_duplication.min_tokens`(하한 = `cpd.minimum_tokens`). 이 판은 §7 예시의 모양을 바꾸지 않으므로 스위트는 초록이어야 한다.

**이 판이 하지 않은 것.** 닫힘 표시, 픽스처, 나머지 key. G0 #1은 통과가 아니다.

### 0-33. v3.9.3 → v3.9.4 (렌즈·모집단 군 — 교차 검토 key 일곱을 한 결정 묶음으로, 2026-09-22)
**넷째 군, 한 판이다.** 일곱 key가 한 질문의 면들이다 — *렌즈 백분위는 어느 집합에서, 무엇에 대해, 언제 정의되는가.* 둘로 가르지 않은 이유: N1·O-4·R1-5/R4-3이 **같은 필드**(`lens_percentiles`·`percentile_population`)의 모양을 바꾼다 — 가르면 같은 필드에 스키마 이행이 두 번, 같은 `expected.json`에 `fixture_change`가 두 번이다. **이 판은 §3의 식을 고치지 않는다** — 식이 어디서 정의되는지를 적는 판이고, §7 예시의 렌즈 값·pct는 하나도 바뀌지 않았다(H 95.6 · Dx 79.3 · F 0.0/96.4 · composite 71.6 · 기존 pct 전부). 바뀐 것은 기록의 모양과 OrderService에 더한 두 값이다.

| key | 절 | 주장 | 어느 면 |
|---|---|---|---|
| N1 (Opus) | §2.5·§3.3·§7 | `percentile_population`이 필드당 칸 하나라 `active`이자 `all`인 파일의 `cx` 백분위 둘 중 하나만 남고 어느 쪽인지 규칙이 없다 | 어느 집합에서 |
| S3 (R3-3·R5-4) | §2.5·§3.3·§3.7 | F의 지시변수가 "√는 단조" 지름길을 깬다 — 곱이 큰데 지시변수 0인 파일이 frozen 파일 위로 오른다 | 무엇에 대해 |
| S10+15 (R1-7·R3-5·R4-7; 15는 R1-7의 쪼갬) | §2.1·§2.2·§2.5·§3.3·§3.4 | "적격"이 정의되지 않았고 소스 세계(모집단)와 클래스 세계(`fan_in`·컴포넌트)가 한 식에서 곱해진다 — 클래스 없는 소스의 `fan_in`이 0으로 내려앉는다 | 어느 집합에서 |
| O-4 (Opus) | §2.5·§3.7 | 렌즈 값은 non-null인데 `lens_pct`가 null(`n_ranked < 20`)이면 `interpretation`이 `context`로 떨어진다 | 언제 |
| R1-5 · R4-3 | §2.5·§3.6·§3.7·§7.1 | composite에 모집단이 없어 백분위가 정의되지 않는데 §2.5는 순위를 요구하고 렌더러는 "렌즈 백분위"로 칠한다 / "렌즈"의 범위가 두 곳에서 다르다 | 무엇에 대해 |
| S14 (R1-4) | §2.5·§3.7 | 같은 사실(무분산)을 §2.5와 §3.7이 다른 시험으로 판정한다 | 언제 |

**정한 것(D168–D173).** (1) 기록은 `<field>.<population>` 두 단계이고 뜻은 **소속** — 렌즈가 쓴 것이 아니라 파일이 속한 모집단이 매긴 것(D168). 그래서 §7에 새 파일을 넣지 않았다: Opus의 "200일 전 커밋 하나면 활성 ∧ F 후보"는 `age ≥ 180`만 봤고 `age > P90_all(age)`를 놓쳤다 — 활성이면 `age ≤ 365`인데 §7의 P90은 900이라 그 예시에서는 성립 불가다. 대신 OrderService에 `cx.all`·`fan_in.all`을 더했다(랭크 302/312·300/340). (2) 지름길은 렌즈별 — F는 지시변수 뒤 0 동점군(D169). (3) 적격 main 소스를 §2.1 표 위에 정의하고 클래스 없는 소스는 `all`에 남기되 클래스 유래 측정은 `null` + `no_bytecode`(D170) — `all` 밖으로 빼면 소스 측정을 클래스 유무로 검열하는 것이다. (4) `interpretation`은 `lens_pct`의 함수이고 `lens_percentiles`는 항상 객체(D171). (5) 렌즈 = H·Dx·F, composite는 렌즈가 아니다(D172). (6) `population_note`는 confidence와 같은 시험(D173).

**포기한 것.** 기록을 렌즈별로(`H.cx`·`F.cx`) 나누는 안 — 소비자가 기준이 되면 같은 (측정, 모집단) 백분위가 렌즈 수만큼 중복되고 `evidence`의 (측정, 모집단) 쌍과 어긋난다. 클래스 없는 소스를 `all` 밖으로 빼는 안 — H에서도 사라진다. composite를 렌즈로 두는 안(모집단 = 세 렌즈 non-null 집합) — 흔들리는 집합 위의 넷째 순위 표면. `lens_percentiles`를 맨 `null`로 두는 안 — 구분이 사라진다.

**이 판이 닫는 key(이름만 — 닫힘 표시는 후속 PR).** N1(D168), S3(D169), S10+15(D170 — 한 key, 15는 R1-7의 쪼갬), O-4(D171), R1-5·R4-3(D172 — 다른 주장 둘을 한 D가 닫되 분리가 아니다), S14(D173). 일곱 전부, 부분 없음. 귀속은 주장으로(D153).

**이 판 뒤 스키마·픽스처가 따라가야 할 자리.** `report.schema.json` — `percentiles`·`percentile_population`의 두 단계 모양(`<field>` → `<population>` → 값/객체), `lens_percentiles.<lens>` 항상 객체(`pct` nullable + `reason?`), `lens_percentiles`에 `composite` 없음(`additionalProperties: false`), `measures.fan_in`·`arch_context` nullable, `reason` 열거에 `no_bytecode`; `contract/schema/` 변조 — "`lens_percentiles.H`가 맨 `null` → 거부", "`percentiles.cx`가 객체가 아니라 수 → 거부"(옛 모양이 새 스키마를 통과하면 이행이 안 된 것), "`lens_percentiles.composite` 있음 → 거부", "`fan_in: null`인데 `reason` 없음 → 거부"; `contract/lens/`(G1) — F의 0 동점군, 두 모집단 파일(`P90_all(age) < 365`인 젊은 리포), `lens_pct` null → `interpretation` null + `reason`. 이 판이 커밋되면 §7 예시의 모양이 바뀌어(`percentiles`가 두 단계, `lens_percentiles`가 객체) 계약 schema가 붉다. **실측(main v3.9.3의 스키마·린트에 대해):** `contract/schema/` 71케이스 중 **36 실패** — `accept-report` 1 + 그 예시에서 파생된 `report-*` 변조 35, 뿌리는 하나(report 예시가 현재 스키마에 44건 어긋남: `percentiles.<field>`가 수가 아니라 객체, `percentile_population.<field>`에 `population`·`n_ranked` 대신 모집단 키). 그리고 **스케일 린트도 8건 붉다** — 예상과 달리. 새 유리수 필드가 아니다: 린트가 필드를 **리프 키**로 매기는데 두 단계 기록의 리프 키는 모집단 이름(`active`·`all`·`dx`)이라 표에 없다고 판정했다(`/files/0/percentiles/cx/active` 0.9741 …, 여덟 전부 4자리 pct). 그래서 D155-1의 표 아래에 "표의 이름은 경로에서 찾는다"를 적었다 — 린트는 `percentiles.<field>.<population>`을 `pct`로 매기는 경로 규칙을 얻어야 한다. 이것은 린트의 가정이 계약의 새 모양을 몰랐던 것이고, D155의 규칙("표에 없는 필드는 표에 먼저")은 그대로다.

**이 판이 하지 않은 것.** 닫힘 표시, 스키마·픽스처, 나머지 key(S8·S11·S12·N6·O-6 — 절이 흩어져 한 줄 판들). G0 #1은 통과가 아니다.
...
### 2.1 파일 단위
**적격 main 소스**(D170) — 모집단(§2.5)의 원소가 되는 파일: 빌드가 main 소스 세트로 선언한 `.java`·`.kt` 중 **테스트 소스 세트 · `@Generated`(javax/jakarta `annotation.Generated`·`annotation.processing.Generated`)가 붙은 최상위 타입 · 빌드가 선언한 생성 소스 디렉터리 유래**를 뺀 것. `bytecode_scope`(§2.2)의 배제 목록을 소스 쪽에 **대칭으로** 둔 것이다. 적격이지만 클래스가 `bytecode_scope`에 없는 소스(`module-info.java`, 주석만 있는 파일, 컴파일 제외 파일)는 **`all`에 남는다** — `cx`·`loc`·H는 소스 측정이라 유효하다 — 대신 클래스 유래 측정(`fan_in`, 컴포넌트 소속)은 `null`이고 `reason: no_bytecode`다. 소스 측정을 클래스 유무로 검열하지 않는다.
...
| `self_dup_tokens` | **발생이 한 파일에만 있는 클러스터**가 덮는 구간의 합집합 토큰 수(§2.6이 정본, D166) | CPD | twin 아님. 혼합 클러스터(A 2회·B 1회)의 A쪽 반복은 여기 들지 않는다 — 그 토큰은 `pair_dup_tokens`에 이미 있어 두 번 셈이 된다 |
...
| `fan_in` | 이 파일 클래스들에 **정적 클래스 그래프에서 직접** 의존하는 프로젝트 내부 클래스 수 | ArchUnit | 테스트 클래스·리플렉션·DI 이름 기반 연결·외부 시스템 호출은 세지 않는다(§2.2 범위). F의 설명에 이 한계를 병기. **클래스가 `bytecode_scope`에 없으면 `null`, `reason: no_bytecode`; null이면 F도 null**(§3.3, D170) — 0으로 내려앉히지 않는다 |
...
| `distinct_authors_90d` | **최근 90일 ∩ 창(W)** 안에서 이 파일을 만진 서로 다른 저자 수 | JGit | **익명 집계**. 저자는 메모리에서 해시로 세고 정체·해시 모두 저장하지 않음. 조정 비용·지식 집중의 지표(Bird 등 2011). `people.attribution=off`에서도 계산 — **off 모드가 내는 저자 유래 필드 셋** 중 하나(D162; 나머지 둘은 `ownership_max_share`·`minor_contributor_share`). **창으로 자르는 이유**(§2.7, D150): 일관성 — 이 표의 이력 성분은 전부 "W 안"이라 90일 지표만 창 밖을 읽으면 "W"가 필드마다 다른 것을 뜻하게 된다 — 와 비용. 재현성은 근거가 아니다(§2.7). 자른 사실은 파일이 아니라 **창**의 속성이므로 `reproduce.window_applied`에 한 번 적는다 |
| `ownership_max_share` | 최다 기여자의 커밋 비중(W 안) | JGit | 익명 집계. 낮으면 소유권 분산. `people.attribution=off`에서도 계산 — 닫힌 목록 셋 중 하나(D162). `= 1.0000`은 "한 사람이 다 만졌다"는 사실이지 누구인지가 아니다(§2.7) |
| `minor_contributor_share` | W 안 커밋 비중 < 5%인 기여자들의 커밋 합 비중 | JGit | 익명 집계. Bird 등의 마이너 기여자 신호. `people.attribution=off`에서도 계산 — 닫힌 목록 셋 중 하나(D162) |
| `team_count` | 이 파일을 만진 팀 수 — 매핑에서 **3인 미만 팀을 `other`로 접은 뒤**의 수(`other`는 하나로 셈) | JGit + 조직 팀 매핑 | `people.attribution ∈ {team, individual}`일 때만; `off`에서는 **필드 부재**(`null`이 아니다, D162). k 하한(≥3인)은 이 팀 집계에 거는 조건이다(D161). 팀 매핑 해시가 이 측정의 캐시 키에 들어간다(D160·§4.5) |
| `teams_folded` | `team_count`를 낼 때 `other`로 접힌 3인 미만 팀의 수 | 조직 팀 매핑 | `team_count`와 함께만(D161). 2인 팀 하나만 만진 파일 → `team_count: 1, teams_folded: 1` — "작은 팀 하나가 만졌다"는 사실은 남고 팀 이름은 남지 않는다. `off`에서는 필드 부재 |
...
### 2.4 파일 쌍 단위
`shared` = **W 안**에서 두 파일을 함께 만진 비머지 커밋 수(D164). `tc = shared / min(chg_commits_a, chg_commits_b)` — 분자와 분모가 같은 구간이라 **`tc ≤ 1`이 불변식**이고 픽스처가 단언한다. 근거는 **일관성**이다(D150과 같은 것 — §2.1 표의 이력 성분 전부가 "W 안"인데 분자만 예외면 "W"가 필드마다 다른 뜻이 된다); 재현성(D87)은 근거가 아니다 — 창 밖 커밋은 SHA 연쇄가 봉인한다(§2.8). 보고: `shared ≥ 5 ∧ tc ≥ 0.5`. 정적 의존 없으면 숨은 결합. `commit_granularity: coarse` 경고 병기.

### 2.5 백분위·분위 계약
**성분 백분위**
- 모집단 셋: `active`(W 안 `chg_commits ≥ 1`) — H; `dx`(`active` ∩ `twins ≥ 1`) — Dx; `all`(모든 **적격 main 소스**, §2.1 표 위의 정의, D170) — F. `cx`는 각 모집단 안에서 언어별. **"렌즈"는 H·Dx·F 셋이다**(D172) — composite는 렌즈가 아니라 프로필의 가중합이다(§3.6).
- 방법: 경험적 CDF, 동점 평균 순위, `pct(x) = (rank_avg(x) − 1)/(N − 1)`. N = 1 → null; N < `percentile.min_population`(20) → null.
- 기록은 **`<field>.<population>` 두 단계**다(D168): `percentiles.<field>.<population>` = 그 모집단에서의 백분위, `percentile_population.<field>.<population> = {lang?, n_ranked, n_population?, reason?}`. **기록의 뜻은 "파일이 속한 모집단마다 그 모집단이 매기는 필드의 백분위"다 — 소속이 기준이지 소비자(렌즈)가 기준이 아니다.** 그래서 `active` 파일은 `cx.active`와 `cx.all`을 **항상** 둘 다 갖는다 — F의 지시변수가 0이어도 `cx`의 `all` 백분위는 존재하고 그 파일은 `all`에 속한다. 필드 이름 하나에 칸 하나면 `active`이자 `all`인 파일의 `cx`가 하나만 남고 어느 쪽인지 규칙이 없다. 파일이 속하지 않은 모집단은 `pct: null` + `reason`으로 기록한다 — 제외된 사실을 남긴다. `evidence`(§3.7)가 이미 (측정, 모집단) 쌍이라 같은 모양이다. `n_ranked` = 공식의 분모 = 그 성분이 non-null인 파일 수, 모집단과 다를 때만 `n_population` 병기 — `age_last_days`처럼 유효값이 모집단보다 적은 성분에서 갈린다. `reason ∈ {not_in_active, no_twins, age_unknown, no_bytecode, insufficient_population}`(D170) — **"값이 null"과 "모집단에서 제외"를 구분**한다.
- 표시: 원값·모집단 분포(중앙값·IQR·최대)와 함께. `population_note`의 조건은 §3.7 confidence의 `low`와 **같은 시험**이다(D173): `IQR = 0 ∨ (median > 0 ∧ IQR < 0.25·median)` — 같은 사실을 두 절이 다른 시험으로 판정하지 않는다(median 0·IQR 0인 모집단이 `low`인데 note가 없는 일이 없다).

**렌즈 백분위 — `interpretation`의 유일한 근거**
- 렌즈 값 자체의 백분위를 그 렌즈의 모집단에서 다시 계산한다: H는 `active`, Dx는 `dx`, F는 `all` — 렌즈는 이 셋뿐이다(D172). null인 렌즈 값은 제외, **0은 포함**. F는 대부분 0이므로 `investigate_first`는 사실상 "F > 0인 파일 중 cx×fan_in 상위" — 의도된 동작.
- 순위·동점·작은 N 규칙은 성분 백분위와 동일. 파일마다 `lens_percentiles.<lens> = {pct, population, n_ranked[, n_population], reason?}` 기록 — **항상 객체**이고 맨 `null`이 아니다(D171): 렌즈 값이 null이거나 랭킹 집합이 작아 백분위가 없으면 `pct: null` + `reason`(성분과 같은 열거 — 렌즈가 null인 이유는 성분이 null인 이유다). 맨 `null`은 "값이 null"과 "모집단에서 제외"의 구분을 지운다. **`n_ranked`는 공식 `(rank_avg−1)/(N−1)`의 분모, 즉 그 렌즈 값이 non-null인 파일 수**이고, 모집단 크기와 다를 때만 `n_population`을 함께 적는다 — 분모가 아닌 수를 인쇄하면 그 자리에서 pct를 재계산할 수 없다(B.4). 성분 백분위의 `n`도 같은 뜻이다. §3.7의 `interpretation`은 이 값만 쓴다 — `H ≥ 90` 같은 절대 임계가 아니다.
...
- **무리수는 크기 출력에만.** 렌즈 값의 `sqrt`(`H = 100·√(pct×pct)`)는 유리수로 닫히지 않는다. √는 **표시할 값에만** `BigDecimal.sqrt(MathContext(34, HALF_EVEN))`으로 계산한다. **순위의 지름길은 렌즈별이다**(D169): **H·Dx**는 √가 단조이므로 `pct × pct`의 정확 비교로 순위를 낸다; **F**는 지시변수 `𝟙[age > P90 ∧ age ≥ 180]`가 곱해져 `pct × pct`의 순서와 F의 순서가 다르므로 **지시변수를 먼저 적용해 0인 파일은 한 동점군(평균 순위)이고 양수끼리만 `pct × pct`로 비교**한다 — 곱이 큰데 지시변수 0인 파일이 진짜 frozen 파일 위로 올라가면 안 된다(§7의 F(OrderService) 150.5/338이 그 0 동점군의 평균 순위다); composite는 아래대로 34자리 값.
- **파생은 전정밀에서, 반올림은 마지막에 한 번.** composite(§3.6 = `0.5·H + 0.3·Dx + 0.2·F`)는 √를 산술 입력으로 쓰므로 위의 "√는 표시값에만"이 닿지 않는다 — **34자리 렌즈 값으로 가중합한 뒤 한 번만 반올림**한다. 표시값으로 더하면 갈린다: 예컨대 H = 95.4934…, Dx = 79.0443…이면 전정밀 경로는 71.4600 → **71.5**, 표시값(95.5·79.0) 경로는 71.45 → **71.4**. composite의 **`priority desc` 정렬**(§3.7)도 √의 가중합이라 `pct × pct` 정확 비교가 통하지 않으므로 34자리 값으로 비교한다 — 정확값은 아니지만 MathContext가 고정이라 재현된다. 이것은 정렬용 비교이지 백분위가 아니다 — composite는 렌즈가 아니라 백분위·`interpretation`이 없다(D172). `contract/lens/`는 두 경로가 갈리는 경계 케이스를 반드시 포함한다.
- **직렬화 — 산출물의 모든 유리수 필드에 스케일을 배정한다**(D155). 렌즈·백분위 둘만 정하고 나머지를 비워 두면 "손으로 쓴 숫자 금지"(§2.9)와 "바이트 동일"이 그 필드들에서 성립하지 않는다 — `expected.json`을 계약에서 생성할 수 없기 때문이다. 스케일은 **고정**이다(`setScale(n, HALF_EVEN).toPlainString()`, 후행 0 유지 — `0.38`이 아니라 `0.3800`). 쓰기·읽기 모두 **`double`을 경유하지 않는다**. 표 `D155-1`:
  | 종류 | 필드 | 스케일 |
  |---|---|---|
  | 정수 측정·개수 | `cx`·`loc`·`file_tokens`·`chg_commits`·`chg_days`·`churn`·`fan_in`·`twins`·`active_twins`·`shared`·`union_dup_tokens`·`self_dup_tokens`·`Ca`·`Ce`·`CCD`·`components`·`n_ranked`·`n_population`·`valid_n`·`unknown_n`·`age_last_days`·`debt_age_days`·`change_exposure_90d`·`authors_window_days`·`distinct_authors_90d`·`team_count`·`teams_folded`(D161) | 정수 |
...
- **자기 중복**: 발생이 한 파일에만 있는 클러스터 → `self_dup_tokens`, twin 아님. 이 문장이 정본이고 §2.1 표의 행은 이것을 말한다(D166) — 분할로 읽는다: `union_dup_tokens`는 파일의 모든 중복 구간, `self_dup_tokens`는 그중 한 파일에만 있는 클러스터가 덮는 구간 — "twin 없이 혼자 복붙한 양". 혼합 클러스터(A 2회·B 1회, 각 100토큰, 겹침 없음)에서 `union_dup_tokens(A) = 200`, `pair_dup_tokens(A,B) = 200`(A쪽 두 구간 합집합), `self_dup_tokens(A) = 0`, `twins(A) = 1`.
...
- 저자 필드: **읽되 정체는 프로세스 밖으로 내지 않는다.** 기본(`people.attribution=off`)은 메모리에서 해시로 세어 **익명 집계 셋만** 낸다 — `distinct_authors_90d`·`ownership_max_share`·`minor_contributor_share`(닫힌 목록, D162; §2.1 표의 비고와 같은 셋이다) — 저자 해시는 저장하지 않는다(재식별 가능). `team_count`·`teams_folded`는 매핑이 필요하므로 off에서는 **필드 부재**이고, 저자 사실은 없다. `team`은 조직 제공 팀 매핑으로 집계하되 **3인 미만 팀은 `other`로 접는다**(D161 — k 하한은 이 팀 집계에 거는 조건이다). `individual`은 저자 사실을 **별도 산출물 `people.json`**에 낸다(D161·§7) — `report.json`이 아니다. 그 저자 사실의 **`author_id`는 조직 팀 매핑 파일이 정한 식별자다 — git 이름·이메일이 아니다**(D163). jQRadar는 git 저자를 그대로 내지 않고 매핑이 준 키로만 낸다(D48의 "추론 금지"와 같은 선 — `individual`을 켠 조직도 자기가 정한 식별자만 받는다); 매핑에 없는 저자는 `unmapped`로 접고 파일마다 `authors_unmapped: n`을 병기한다 — 팀의 `other`·`teams_folded`와 같은 처리다. 값은 있고 정체는 없다. 어떤 모드에서도 `.jqradar/`(커밋 대상)에는 정체를 저장하지 않고, `people.json`도 거기 있지 않는다. **모드와 매핑은 id 입력이다**(D160): `people.attribution`과 팀 매핑 파일의 해시 `team_mapping_sha256`이 `analysis_input_id`에 들어가고 `reproduce.people`로 인쇄된다 — 매핑 파일의 해시는 정체가 아니다: 저자 해시(사람 하나에 대응해 재식별 가능)와 달리 파일 하나의 해시이고 `check` 프로필의 `policy_hash`와 같은 부류다. **k 하한은 익명 집계에 걸지 않는다**: `distinct_authors_90d = 1`은 "한 사람이 만졌다"는 사실이지 누구인지가 아니다 — 재식별에는 외부 조인(`git log`)이 필요하고 그것은 저자를 직접 보여 주니 이 필드가 더하는 것이 없다(D108의 PR 번호와 같은 논거). k가 거는 자리는 **팀 라벨**뿐이다 — 사람의 묶음이라 라벨이 곧 정체에 가깝기 때문이다(D162). `declared_ai_assistance`는 커밋 트레일러(`Co-authored-by:` 봇, `Generated-by:`)에서 **선언된 것만** 읽고 추론하지 않는다. blame: **매 스캔에서는 사용하지 않는다.** 유일한 예외는 §5.6 — 캠페인 생성 시 원장 finding 영역에 한정해 1회. 매 스캔 경로에서 blame이 호출되면 픽스처가 실패한다.
...
| `schema/` | 산출물 스키마 **8종**(report·change·gate·validate·campaign·event·fixture_change·**people** — D161)마다 **긍정 1 + 거부되어야 할 변조 ≥ 1**. 변조는 구조 오류만이 아니라 **하드룰**을 겨눈다 — 사건에 정체 필드(D48), 원장 사건에 코드 상태(D55), `change` 항목에 판정 단어·점수(B.2), `validated`인데 `validated_tree_id` 없음(D89). **하드룰은 한 스키마에만 걸면 뚫린다**: `report`의 파일·finding에 `author`·`severity`·`score`를 넣는 변조가 각각 거부되어야 하고(D126), 자유 텍스트(`population_note`·`zone_note`·`composite_note`)의 판정 어휘도 렌더러와 **같은 사전**으로 막는다(D127). 직렬화 스케일은 스키마가 아니라 **원문 텍스트 린트**로(D125) — 린트는 §2.5 표 `D155-1`의 **모든 유리수 필드**를 보고, 표에 없는 유리수 필드가 나오면 실패한다(D155). **렌즈·모집단 기록의 모양**(D168·D171·D172·D170): `lens_percentiles.H`가 맨 `null` → 거부, `percentiles.cx`가 객체가 아니라 수 → 거부(옛 모양이 새 스키마를 통과하면 이행이 안 된 것), `lens_percentiles.composite` 있음 → 거부, `fan_in: null`인데 `reason` 없음 → 거부 | G0 |
| `reproducibility/` | 같은 트리·다른 PMD 버전 → `analysis_input_id` 다름·`repository_state_id` 같음; 파라미터 하나 변경 → id 변경; `reproduce`에 없는 환경 변수를 바꿔도 결과 불변(원칙 검사); 두 머신 바이트 동일; **full clone vs `--depth 3` shallow clone(같은 트리·같은 창 안 커밋 목록) → `last_commit_map_sha256`이 달라 id 다름 ∧ shallow는 `age_unknown` ∧ `history_complete: false`**(D146·D148·D149); **"두 머신 바이트 동일"의 비교 대상**(D154) = 산출물에서 `reproduce.scanned_at`·`reproduce.classes_reproduction_inputs`를 **뺀 전부** — 두 실행의 산출물이 그 규칙으로 동일하고 제외 필드는 실제로 다름(`scanned_at`)을 함께 단언한다; **파라미터 하나 변경**(`cpd.minimum_tokens`) → 그 파라미터를 읽는 측정(`pair_dup_tokens`)만 캐시 미스, 다른 측정은 적중(D159); **시각 표기**: 같은 unix 초를 `Z`와 `+09:00`으로 적은 두 입력이 정규 표기로 같은 바이트(D157); **`people.attribution` 토글**(off → team) → `analysis_input_id` 변경 ∧ `repository_state_id` 불변(D160); **매핑 한 줄 수정** → `team_count`의 캐시 키만 미스, `cx`·`pair_dup_tokens` 적중(D160·D159) | G0 |
| `cpd/` | 겹침 합집합(150), 무순서 쌍, 자기 중복, 임계 경계, **같은 열 4발생 = 클러스터 1개**, **혼합 클러스터**(A 2회·B 1회, 각 100토큰, 겹침 없음 → `union_dup_tokens(A) 200`·`pair_dup_tokens(A,B) 200`·`self_dup_tokens(A) 0`·`twins(A) 1`, D166) | G0 |
| `history/` | 앵커 고정(같은 HEAD 다른 날짜), 머지 제외, rename(tie-break·`rename_ambiguous`), 상한 두 종류, coarse, **shallow clone → `age_unknown`**(`.git/shallow` 명시 감지 — 감지 없이 200 같은 그럴듯한 수가 나오면 실패), **창 밖 마지막 커밋 복구**(창 안 커밋이 없는 파일의 `age_last_days`가 한정 걷기로 나온다 — §7의 1250이 이 경로다, D147·D149), **2인 팀 하나만 만진 파일 → `team_count: 1, teams_folded: 1`**(D161), **off 산출물에 닫힌 목록 셋 밖의 저자 유래 필드가 없음**(D162 — 부재 검사이므로 D131대로 `must_be_caught`(`team_count`·`author`가 있으면 잡힘)와 `must_not_be_caught`(`distinct_authors_90d`·`ownership_max_share`·`minor_contributor_share`는 잡히면 안 됨)를 함께), **`shared`의 창 한정**(창 밖에 두 파일을 함께 만진 커밋이 있어도 `shared`·`tc` 불변 ∧ 모든 쌍에서 `tc ≤ 1`, D164), **숨은 결합의 정렬**(정적 의존 있음·`tc` 높음 vs 없음·`tc` 낮음, `tc` 동점 두 쌍 → 인쇄 순서 고정, D167), 음수 나이 → `invalid_metadata`, **`scan`·`change` 경로에서 blame 호출 시 실패**(부재 검사이므로 `must_be_caught` + `must_not_be_caught`). **§2.4 파일 쌍**: `shared`·`tc = shared / min(chg_commits_a, chg_commits_b)`·보고 임계(`shared ≥ 5 ∧ tc ≥ 0.5`)·정적 의존 유무의 부착, 그리고 **같은 작업을 스쿼시/비스쿼시로 만든 쌍**(분모가 작아져 `tc`가 커지는 것을 값으로 고정한다 — 판정은 없고 `commit_granularity: coarse` 표기만). **§2.1 저자 사실**: 커밋별 저자가 다른 합성 리포에서 `distinct_authors_90d`·`ownership_max_share`·`minor_contributor_share`(`team_count`는 팀 매핑이 있을 때), 그리고 **`attribution=off`에서 정체가 산출물에 없음** — 이것은 부재 검사이고 모양이 아니라 **출처**로 가른다(이 표 뒤의 "부재를 보는 검사" 문단). **90일 지표의 창 절단**: 커밋 수 상한이 90일보다 짧은 구간을 남기는 리포에서 창 밖 커밋이 `distinct_authors_90d`를 움직이지 않는다(§2.7·D137) — `authors_window_truncated`·`authors_window_days` 표기와 함께, 상한이 넉넉한 변이를 대조로 | G0 |
| `graph/` | 외부 의사 노드, auto 전략 4종, 순환, NCCD 검산 | G0 |
| `lens/` | H/Dx/F 각 모집단, composite null, confidence 최소 집계, tie-break, **F null(나이 미상·`no_bytecode`)**, **composite 전정밀 vs 표시값 경로가 갈리는 경계 케이스**, §7 `sample-service`(랭크 배정이 입력), **F의 0 동점군**(곱이 큰데 지시변수 0인 파일이 F = 0 동점군에 남는다, D169), **두 모집단 파일**(`active` ∧ F 지시변수 1 — `P90_all(age) < 365`인 젊은 리포에서만 성립, D168), **`lens_pct` null → `interpretation` null + `reason`**(`n_ranked < 20`, D171) | G1 |
| `gate/` | BASE 자격 고정, 모집단 드리프트 무영향, **파일×클러스터 수준 새 중복**(D165): BASE A 1·B 1 → HEAD A 2·B 1 → `new_duplication` 1(A); A↔B → A↔C → 1(C); B→C rename은 0; 자기 중복 클러스터 → 0; `gate.new_duplication.min_tokens < cpd.minimum_tokens`인 프로필 → 거부 | G1 |
...
모집단 **`all`**(W 활동 없는 파일이 바로 F의 후보다). 시간 기준은 `HEAD_TIME`. `P90`은 §2.5의 type-7 분위, 유효 나이만. F의 후보는 정의상 **마지막 커밋이 창 밖인 파일**이므로 `age_last_days`는 §4.5의 한정 걷기가 창 이전에서 복구하고(D147), 그 마지막 커밋은 `analysis_input_id`의 입력이다(D146) — 창 안만 걷거나 창 안만 id에 넣으면 F는 모집단 정의에서 그랬듯(§0-4) 찾을 대상을 잃거나 재현되지 않는다. **이름보다 약하게 읽어야 한다**: F는 "복잡하고 정적 fan-in이 높고 오래 안 바뀐 코드"의 대리 지표 — **변경 위험 후보**이지 부채 판정이 아니다. 안정된 도메인 코어, 의도적으로 고정한 API, 테스트가 두터운 옛 코드가 다 여기 걸린다. 부채가 되는 것은 조직이 캠페인에 등록할 때다. **`age_last_days = null`이면 F = null** — "오래되지 않았다"와 "나이를 모른다"는 다른 상태이므로 0을 주지 않는다. **`fan_in = null`(클래스가 `bytecode_scope`에 없다, `reason: no_bytecode`)이면도 F = null**(D170) — 같은 이유다: "의존하는 클래스가 없다"와 "클래스가 없어 셀 수 없다"는 다른 상태다. F의 순위는 지시변수를 먼저 적용해 0을 한 동점군으로 둔 뒤 양수끼리 `pct × pct`로 낸다(§2.5, D169).

### 3.4 Architecture Context — 파일 점수가 아니라 맥락
파일마다 소속 컴포넌트의 `I, A, D, zone, cycle_id`를 **붙인다**. 곱하거나 더하지 않는다. 클래스가 `bytecode_scope`에 없는 소스(§2.1 적격 정의)는 소속 컴포넌트가 없으므로 이 맥락이 **`null`**이다(D170) — 컴포넌트는 클래스 세계의 것이고 그 파일은 소스 세계에만 있다. 리포트에서는 컴포넌트 단위로 따로 보인다: D 상위 컴포넌트, 순환 목록(끊기 후보 간선 포함), NCCD(시스템 숫자 하나, 스캔 간 추세로만 — 이력 저장소 전까지는 두 점). `zone` 표기는 위치 설명이며 정당한 예외(유틸리티·도메인 코어)를 항상 병기한다(§2.2).

### 3.5 Hidden Coupling — 점수가 아니라 목록
`shared ≥ 5 ∧ tc ≥ 0.5`인 파일 쌍을 **정렬 키** `static_dependency asc(false 먼저) · tc desc · shared desc · a asc · b asc`로 — "정적 의존이 없는 쌍 우선"은 첫 키를 말한 것이지 다른 규칙이 아니고, 비교는 반올림 전 정확값(§2.5)이다(D167). `hidden_couplings`는 §7에서 배열이라 순서가 바이트다(D154) — 이 키가 없으면 두 머신이 같은 쌍을 다른 순서로 인쇄한다. 각 행에 `shared`·`chg_commits_a/b`·정적 의존 유무·`commit_granularity` 경고. 테스트-대상 쌍은 "test pair"로 표시하고 긍정 해석을 병기한다 — 파일이 진화하며 테스트도 갱신되고 있다는 신호일 수 있다. 판정하지 않는다(§2.4, §7.1 A5).

### 3.6 복합 점수(선택)
`--profile` 가중합, 기본 `H 0.5, Dx 0.3, F 0.2`. **세 렌즈가 모두 non-null일 때만 계산**, 아니면 `null` + `composite_note`(재정규화는 부풀리고 0 대입은 깎으므로 둘 다 하지 않는다). 헤드라인은 H. **composite는 렌즈가 아니다**(D172): 백분위(`lens_percentiles.composite`)·`interpretation`·`anchor_class`가 없다. 이유 둘 — 모집단이 "세 렌즈가 모두 non-null인 집합"이라 프로필과 리포마다 흔들리는 집합이고, 그 위에 백분위를 두면 넷째 순위 표면이 된다(B.2·B.3이 막는 점수의 모양). composite의 `priority desc` 정렬은 34자리 값 비교다(§2.5).

### 3.7 priority · interpretation · confidence
- `priority` = 렌즈 값. 정렬 tie-break: `priority desc, path asc, id asc`.
- `interpretation`은 **`lens_percentiles.<lens>.pct`의 함수**다(D171): `≥ 0.90` → `investigate_first`, `≥ 0.75` → `investigate`, 그 외 `context`; **`pct`가 null이면 null**이고 같은 `reason`(`not_in_active`·`no_twins`·`age_unknown`·`no_bytecode`·`insufficient_population`)이 왜인지 말한다 — 렌즈 값은 non-null인데 랭킹 집합이 작아(`n_ranked < 20`) 백분위가 없는 파일이 `context`로 떨어지면 안 된다. `anchor_class`(§5.5)도 `lens_pct`에서 나오므로 같은 경로로 null + 같은 `reason`이다. 백분위 임계이며 개수 자르기가 아니다.
...
- `check`: `jqradar.yml`(선택, 정책). `campaign create`: 인자로 조직이 적는 정의(앵커·범위·목표). **`people` 절**(`jqradar.yml` 안): `attribution: off(기본) | team | individual`, 팀 매핑 경로. **이 셋이 조직이 적는 자리다**(B.6). 전부 이름·소유자·해시가 산출물에 인쇄된다. `scan`은 `jqradar.yml`의 `people` 절만 읽는다 — 무설정의 유일한 예외이고, **그 값은 id 입력이다**(§2.8, D160). "정책이지 파라미터가 아니다"는 조직이 적는 자리의 분류(B.6)이지 결과를 바꾸는 입력의 면제가 아니다 — 모드가 다르면 산출물이 다르고, 그래서 `analysis_input_id`가 다르다.
...
### 5.2 새 중복의 정체성
클러스터 해시만으로는 부족하다: `BASE: A↔B`, `HEAD: A↔C`는 같은 클러스터 해시라 "기존"으로 보여 새 사본 C를 놓친다. **파일×클러스터 수준**으로 정의한다(D165):
> `new_duplication` = **서로 다른 파일 ≥ 2**인 HEAD 클러스터에 대해, 파일×클러스터 `(rename 매핑 후 경로, 클러스터 ID)`별 **새 발생 수** = `max(0, HEAD 발생 수 − BASE 발생 수)`. 단순 rename(B→C, 내용 동일)은 매핑이 흡수한다.

키에 오프셋을 넣지 않는 이유: 오프셋은 무관한 편집에도 흔들려 같은 사본이 새것으로 보인다. 그러나 **같은 파일 안 추가 사본은 센다 — 오프셋이 아니라 개수로**: A↔C(C의 BASE 발생 수 0 → 1)도, A 안의 둘째 사본(1 → 2)도 잡힌다. "서로 다른 파일 ≥ 2"인 이유: §2.6에서 클러스터는 정의상 발생 ≥ 2라 "발생 ≥ 2"는 아무것도 거르지 못하고, 한 파일 안에서만 도는 클러스터(자기 중복)는 `self_dup_tokens`의 자리이지 새 중복이 아니다 — §2.6의 twin 정의와 같은 선이다.
...
| 조건 | 대상 | 기본값 |
|---|---|---|
| `new_duplication`이 변경 파일에 존재 | 변경 파일 | 클러스터 토큰 ≥ `gate.new_duplication.min_tokens` → FAIL. **기본값 = `cpd.minimum_tokens`(100) — 기본에서는 아무것도 거르지 않는다**; 조직이 올릴 수 있는 손잡이이고 하한은 `cpd.minimum_tokens`(그 아래는 뜻이 없어 프로필이 거부된다, D165) |
...
**픽스처 입력은 랭크 배정이다.** 백분위는 측정값 하나로 나오지 않으므로 `contract/lens/sample-service/input.json`이 각 성분·렌즈의 `(rank_avg, n_ranked)`를 정하고, 아래 값은 전부 거기서 계산된다 — 손으로 고른 pct가 아니다(B.4·D26). 이 예시의 배정: `cx` 189/194 · `chg_commits` 199/212 · `union_dup_tokens` 33/37 · `active_twin_ratio` 26.5/37(동점) · `cx(OrderPolicy)` 296/312 · `fan_in(OrderPolicy)` 333/340 · **`cx(OrderService, all·java)` 302/312 · `fan_in(OrderService)` 300/340**(D168 — `active` 파일은 `all`의 백분위도 갖는다; F는 지시변수 0이라 그대로 0) · 렌즈 `H` 197/212 · `Dx` 30/37 · `F(OrderService)` 150.5/338(0 동점군) · `F(OrderPolicy)` 335/338.
...
```jsonc
{
  ...
  "people_artifact": null,           // attribution ∈ {team, individual}이면 "people.json"(D161) — off에서는 null. 저자 사실은 report에 오지 않는다(D126)
  "reproduce": {                    // 필드는 세 종류뿐(D158·§2.8): [id] id 입력 · [fn] id 입력의 결정적 함수 · [x] 비교 제외(D154)
    ...
    "people": { "attribution": "off", "team_mapping_sha256": null }   // [id] 정책이지만 산출물을 바꾸므로 id 입력(D160). parameters 안이 아닌 이유는 B.6 — 조직이 적는 자리가 다르다
  },
  "population": {
  ...
  "files": [
    { "id": "f:order-service", "path": "src/main/java/…/order/OrderService.java", "component": "….order", "lang": "java",
      ...
      "percentile_population": { "cx": {"active": {"lang":"java","n_ranked":194}, "all": {"lang":"java","n_ranked":312}}, "chg_commits": {"active": {"n_ranked":212}},
                                 "union_dup_tokens": {"dx": {"n_ranked":37}}, "active_twin_ratio": {"dx": {"n_ranked":37}}, "fan_in": {"all": {"n_ranked":340}} },
    ...
    { "id": "f:order-policy", "path": "src/main/java/…/order/OrderPolicy.java", "component": "….order", "lang": "java",
      ...
      "percentile_population": { "cx": {"all": {"lang":"java","n_ranked":312}}, "fan_in": {"all": {"n_ranked":340}},
                                 ...
                                 "union_dup_tokens": {"dx": {"n_ranked":37,"reason":"no_twins"}}, "active_twin_ratio": {"dx": {"n_ranked":37,"reason":"no_twins"}} },
      "lenses": { "H": null, "Dx": null, "F": 96.4, "composite": null, "composite_note": "H,Dx null — not in active/dx population" },
      "lens_percentiles": { "H": {"pct": null, "population": "active", "reason": "not_in_active"}, "Dx": {"pct": null, "population": "dx", "reason": "no_twins"},   // 항상 객체 — 맨 null 아님(D171)
                            "F": {"pct": 0.9911, "population": "all", "n_ranked": 338, "n_population": 340} },
      "arch_context": { "I": 0.3800, "A": 0.0000, "D": 0.6200, "zone": "stable-concrete", "cycle_id": "cyc-1" } }
  ],
...
```jsonc
{ "schema": "jqradar/people/1", "analysis_input_id": "sha256:…",      // 어느 분석의 것인가 — report와 같은 id
  "attribution": "team",                                              // team | individual
...
| A2 | **aspect 토글** | 버튼: H · Dx · F · composite(프로필 있을 때) · arch(컴포넌트 zone·SCC 강조) · lang(언어 분포) · confidence(불투명도). 복수 선택 시 각 렌즈 백분위를 **색 채널로 겹침**(가중합 아님 — 시각적 중첩만, 점수를 만들지 않는다). composite 토글은 백분위가 아니라 **값(0–100)**을 칠한다 — composite는 렌즈가 아니라 백분위가 없고(D172), 렌더러는 JSON에 없는 수를 만들지 않는다(D28). 선택 상태는 URL 해시에 저장·복원 | Combined Aspects |
...
1. **§2.1–2.9** 계약의 **교차 검토에서 열린 발견이 0**이다(D140). 통과 조건은 리뷰어의 종류가 아니라 발견의 수다. 검토 주체는 **다중 모델 패널**(D141): 서로 다른 실행 모델 최소 넷, 같은 입력(`docs/review/g0-review.py --pairs`), 같은 세 질문, 같은 판정 셋(모순 없음 / 모순 / 모호), 각 모델이 쌍 목록 **밖**에서 절 둘을 직접 고른다. 패널은 **표본 생성기이지 판정기가 아니다** — 만장일치는 통과 근거가 아니고, 한 모델의 "모순 없음"은 그 모델이 못 본 것으로만 기록하며, 모델별 판정을 **따로** 적는다(합의만 적으면 불일치라는 신호가 사라진다). "사람 2명·외부 1명" 요구는 G0에서 뺀다 — D84가 지키는 것(자기 적용이 외부 검증을 대체하지 못한다)이 실제로 시험되는 자리는 G3의 외부 배터리·외부 파일럿·사람 검증자이고(D142), 안에서 반증할 수 없는 논거를 닫을 수 있는 관문에 걸지 않는다. 패널 산출물은 여전히 **내부자 데이터**다. **§6.3–6.6은 여기서 빠진다** — 그 계약들의 값 층 픽스처가 G3(`contract/validation·security`)에 오므로 지금 검토하면 문장만 읽게 되고, "검토했다"가 "문장만 읽었다"를 감춘다. G3 입장 조건으로 옮긴다(§9 일정). **"열린 발견 0"은 다음 규칙으로 센다**(D151):
   ...
   - **열린 발견 0** = 범위 안의 모든 `dedupe_key`가 **D 번호로 닫혔음이 기록에 적혀 있다.** 이것이 D140의 0이고, 그래서 `records/`를 읽어 세는 것이 가능하다 — G0 #1의 상태는 사람 보고가 아니라 계산이다(`g0-review.py --count`). 닫힘 표시는 **기록에서만** 읽는다 — `prd.md`를 읽어 추측하지 않는다.
     - **닫힘의 단위는 key이고 상태는 둘뿐이다**(D153). 한 key는 여러 D로 닫힐 수 있다 — `닫힘 — D146·D147·D148`처럼 적고, 계산기는 표시의 유무만 본다. **"부분 닫힘"이라는 셋째 상태는 만들지 않는다.** D가 key의 일부만 닫았다면 그 key의 "주장 하나"가 실은 둘이었다는 뜻이고 — dedupe가 덜 된 것이다 — 그때는 key를 **쪼갠다**: 닫힌 부분을 새 key로 떼어 `닫힘 — D<n>`, 남은 부분은 원래 key로 열린 채. 상태를 늘리면 계산기가 "열림/닫힘/부분"을 세게 되고 "부분"이 곧 판단이 되어 계산이 다시 사람 보고로 돌아간다.
     - **쪼갬은 종합자가 아니라 닫는 결정이 한다.** 종합자의 쪼갬은 막는다(한 개별이 종합 둘에 붙으면 한 key로 접는다 — 접지 않으면 개별 하나를 셋으로 쪼개 발견 수를 늘릴 수 있다). 결정의 분리는 허용한다. 둘을 계산기가 가르는 유일한 표지는 **D 번호**다 — 대응표의 "결정에 의한 분리" 행에는 D 번호가 반드시 붙고, D 번호 없는 분리는 쪼갬으로 접는다. 닫힘의 귀속은 **주장**으로 한다, 절 겹침으로 하지 않는다 — §2.7을 함께 가리키는 두 key라도 주장이 다르면(예: 머지 제외의 집합 vs 90일 절단) 한쪽을 닫은 D가 다른 쪽을 닫은 것이 아니다.
...
- D152 [§9] D151의 `dedupe_key`를 보충한다(개정 아님). (D153이 닫힘의 단위와 분리를 보충) **절 집합의 출처** = 발견이 원문을 인용한 문장이 속한 절 전부 — 리뷰어가 선언한 쌍의 넓이가 아니다; 기록에서는 발견 항목의 `절:` 줄이 그것이고 판정표의 쌍 표기는 아니며, 종합의 각 `dedupe_key`는 병합한 개별 발견(파일·번호)을 가리킨다. **걸침** = 절 집합에 §2.1–2.9의 절이 하나도 없으면 범위 밖, 그 외는 열린 동안 범위 안; 닫힐 때 D의 대괄호가 착지를 기록하고 전부 밖에 착지했으면 "범위 밖 착지"로 표시한다(이미 닫힌 것이라 0의 계산에 무영향). 이유: 고침의 자리로는 미리 분류할 수 없고(순환), 결정 자리를 옮기는 탈출구를 막고, 고침 자리가 뻔한 것도 결정을 강제한다.
...
**v3.9.1 결정 — 저작 군(교차 검토 key 셋: S2·S9·R4-6)**
- D160 [§2.8, §2.7, §4.3, §4.5, §7, §2.9] **`people.attribution`과 팀 매핑 파일의 해시 `team_mapping_sha256`은 §2.8 id 입력이다** — 모드는 산출물을 바꾸고, "정책이지 파라미터가 아니다"(B.6)는 조직이 적는 자리의 분류이지 결과를 바꾸는 입력의 면제가 아니다. `reproduce`에 **별도 `people` 블록**(`[id]`, D158)으로 인쇄하고 `off`에서도 인쇄한다(`team_mapping_sha256: null`). 매핑 파일의 해시는 정체가 아니다(파일 하나의 해시 — `policy_hash`와 같은 부류). 사영(D159): 리포트 키에는 들어가되 측정 캐시에서는 `team_count`와 저자 사실만 매핑 해시를 읽는다.
- D161 [§2.7, §2.1, §2.5, §2.9, §7] 저자 사실은 **8번째 산출물 `people.json`**으로 간다(`schemas/`가 8종이 된다). `report`의 `author` 거부(D126)는 조건 없이 그대로다 — 조건부로 열면 변조가 "조건 없을 때"만 겨눠 방어가 약해지고, MCP 응답에만 두면 켠 사람이 볼 수 없다. `people.json`은 `attribution ∈ {team, individual}`에서만 생성되는 `build/` 산출물이고 `.jqradar/`에는 절대 없다(D48); `report.json`은 `people_artifact`로 존재만 가리키고 `off`에서 이 파일이 있으면 계약 위반이다. **k 하한(≥3인)은 팀 집계의 조건**: 매핑에서 3인 미만 팀은 `other`로 접고, `team_count`는 접은 뒤의 수(`other`는 하나로 셈), `teams_folded`를 병기한다 — 값은 있고 정체는 없다.
- D162 [§2.7, §2.1, §2.9] **off 모드가 내는 저자 유래 필드는 셋으로 닫힌다**: `distinct_authors_90d`·`ownership_max_share`·`minor_contributor_share`. `team_count`·`teams_folded`는 off에서 **필드 부재**(`null`이 아니다), 저자 사실은 없다. **k 하한은 익명 집계에 걸지 않는다** — `distinct_authors_90d = 1`은 "한 사람이 만졌다"는 사실이지 누구인지가 아니고, 재식별에 필요한 외부 조인(`git log`)은 저자를 직접 보여 주므로 이 필드가 더하는 것이 없다(D108과 같은 논거). k가 거는 자리는 팀 라벨뿐이다.

**v3.9.2 결정 — `author_id`(한 줄 판)**
- D163 [§2.7, §7] `people.json`의 **`author_id`는 조직 팀 매핑 파일이 정한 식별자다 — git 이름·이메일이 아니다.** jQRadar는 git 저자를 그대로 내지 않고 매핑이 준 키로만 낸다(D48의 "추론 금지"와 같은 선). 매핑에 없는 저자는 `unmapped`로 접고 파일마다 `authors_unmapped: n`을 병기한다(팀의 `other`·`teams_folded`와 같은 처리, D161). 식별자의 **형식**은 이 판이 정하지 않는다 — 매핑 파일이 정하는 조직의 것(B.6)이라 스키마는 `type: string`과 "매핑 파일의 키"라는 조건만 갖는다.

**v3.9.3 결정 — 쌍·순서 군(교차 검토 key 넷: S7·S4·R2-5·R1-9)**
- D164 [§2.4, §2.9] `shared` = **W 안**에서 두 파일을 함께 만진 비머지 커밋 수. 분자·분모가 같은 구간이라 `tc ≤ 1`이 불변식이다. 근거는 일관성(§2.1의 이력 성분 전부가 "W 안") — 재현성은 근거가 아니다(D150).
- D165 [§5.2, §5.3, §2.9] 새 중복은 **파일×클러스터 수준**: 서로 다른 파일 ≥ 2인 HEAD 클러스터에 대해 `(rename 매핑 후 경로, 클러스터 ID)`별 **새 발생 수** = `max(0, HEAD − BASE)`. 오프셋은 키에 넣지 않고(무관한 편집에 흔들린다) 같은 파일 안 추가 사본은 개수로 센다. "발생 ≥ 2"는 거르는 것이 없어 "서로 다른 파일 ≥ 2"로 — 자기 중복은 새 중복이 아니다. 게이트의 토큰 조건은 `gate.new_duplication.min_tokens`, 기본값 = `cpd.minimum_tokens`(기본에서는 거르지 않는다), 하한은 `cpd.minimum_tokens`.
- D166 [§2.6, §2.1, §2.9] `self_dup_tokens`는 **발생이 한 파일에만 있는 클러스터**가 덮는 구간의 합집합 토큰 수 — §2.6이 정본이고 §2.1 표의 행이 그것을 말한다. 혼합 클러스터의 같은 파일 반복은 들지 않는다(`pair_dup_tokens`에 이미 있어 두 번 셈). 분할: `union_dup_tokens` ⊇ 교차 파일 구간 ∪ 자기 중복 구간.
- D167 [§3.5, §2.9] `hidden_couplings`의 정렬 키 = `static_dependency asc(false 먼저) · tc desc · shared desc · a asc · b asc`, 비교는 반올림 전 정확값. 배열이라 순서가 바이트다(D154). 정렬은 인쇄되는 키이지 점수가 아니다(B.2).

**v3.9.4 결정 — 렌즈·모집단 군(교차 검토 key 일곱: N1·S3·S10+15·O-4·R1-5·R4-3·S14)**
- D168 [§2.5, §7] 백분위 기록은 **`<field>.<population>` 두 단계**(`percentiles.cx.active`·`percentiles.cx.all`, `percentile_population.cx.all = {lang?, n_ranked, n_population?, reason?}`). 기록의 뜻은 "파일이 속한 모집단마다 그 모집단이 매기는 필드의 백분위" — 소속이 기준이지 소비자가 기준이 아니다. `active` 파일은 `cx.active`·`cx.all`을 항상 둘 다 갖고, 속하지 않은 모집단은 `pct: null` + `reason`으로 기록한다. §7의 OrderService에 `cx(all)` 302/312 → 0.9678, `fan_in(all)` 300/340 → 0.8820을 더했다 — 렌즈 값은 하나도 바뀌지 않는다. "활성 ∧ F 지시변수 1"인 파일은 `P90_all(age) < 365`인 젊은 리포에서만 성립하므로(활성이면 `age ≤ 365`) §7에는 넣지 않고 `lens/`(G1)의 자리다.
- D169 [§2.5, §3.3, §2.9] 순위의 지름길은 **렌즈별**: H·Dx는 `pct × pct` 정확 비교(√ 단조); **F는 지시변수를 먼저 적용해 0인 파일은 한 동점군(평균 순위), 양수끼리만 `pct × pct` 비교**; composite는 34자리 값. §7의 F(OrderService) 150.5/338이 그 0 동점군의 평균 순위다.
- D170 [§2.1, §2.5, §3.3, §3.4] **적격 main 소스** = 빌드가 main 소스 세트로 선언한 `.java`·`.kt` 중 테스트 소스 세트·`@Generated` 최상위 타입·빌드가 선언한 생성 소스 디렉터리 유래를 뺀 것 — `bytecode_scope`의 배제 목록을 소스 쪽에 대칭으로. 클래스가 `bytecode_scope`에 없는 적격 소스는 **`all`에 남고**(소스 측정은 유효) `fan_in: null, reason: no_bytecode`, 컴포넌트 소속 `null`, `fan_in`이 null이면 F도 null. `reason` 열거에 `no_bytecode`.
- D171 [§3.7, §2.5, §7] `interpretation`은 **`lens_percentiles.<lens>.pct`의 함수** — `pct`가 null이면 null이고 같은 `reason`이 왜인지 말한다. `lens_percentiles.<lens>`는 **항상 객체**(맨 `null` 아님 — "값이 null"과 "모집단에서 제외"의 구분을 지운다). `reason`은 성분의 열거를 그대로 쓴다. `anchor_class`(§5.5)도 같은 경로로 null + 같은 `reason`.
- D172 [§2.5, §3.6, §7.1] **"렌즈" = H·Dx·F**, §2.5 한 곳에서 정의한다. **composite는 렌즈가 아니다** — 백분위·`interpretation`·`anchor_class`가 없다(모집단이 흔들리는 집합이고, 그 위에 백분위를 두면 넷째 순위 표면이 된다 — B.2·B.3). §2.5의 composite 순위 문장은 `priority desc` 정렬용 비교로 좁힌다. §7.1 A2의 composite 토글은 값(0–100)을 칠한다(D28). R1-5와 R4-3은 다른 주장(모집단 / 낱말의 범위)이라 한 D가 둘을 닫되 분리가 아니다(D153).
- D173 [§2.5] `population_note`의 조건 = confidence `low`와 같은 시험: `IQR = 0 ∨ (median > 0 ∧ IQR < 0.25·median)`.
