...
- O14 (v3.8.4) **열린 발견 0의 정지 규칙.** D140의 조건은 "그만 찾으면 0"이 되는 조건이다. 언제 "더 없다"고 말할 수 있는가 — 후보: (a) 마지막 패널 라운드(모델 넷)에서 **새로** 나온 발견이 0이고 목록 밖 쌍에서도 0, (b) 라운드를 두 번 연속 반복해 (a)가 유지, (c) 라운드마다 쌍 목록을 바꾼다(같은 입력의 반복은 같은 맹점을 반복한다). 결정 전까지 §12의 임시 읽기("마지막 라운드의 새 발견 0")를 쓴다. 이 결정이 없으면 D140은 굿하트에 열려 있다.
...
- **G0 #1의 새 조건이 재는 것은 발견의 수이지 판정자의 독립성이 아니다.** "열린 발견 0"은 그만 찾으면 0이 된다(굿하트). 그래서 정지 규칙이 필요하고(O14), 그것이 없는 동안 G0 #1은 "패널 마지막 라운드에서 **새로** 나온 발견이 0"으로 읽는다 — 닫힌 것의 수가 아니라 나온 것의 수다. 조건을 바꾼 판에서 통과를 선언하지 않는다(§0-23).
