...
| `distinct_authors_90d` | **최근 90일 ∩ 창(W)** 안에서 이 파일을 만진 서로 다른 저자 수 | JGit | **익명 집계**. 저자는 메모리에서 해시로 세고 정체·해시 모두 저장하지 않음. 조정 비용·지식 집중의 지표(Bird 등 2011). `people.attribution=off`에서도 계산. **창으로 자르는 이유**(§2.7): 창 밖 커밋은 `analysis_input_id`의 입력이 아니므로(§2.8) 값에 영향을 주면 같은 id가 다른 값을 낸다 — D87이 막는 자리다. 자른 사실은 파일이 아니라 **창**의 속성이므로 `reproduce.window_applied`에 한 번 적는다 |
...
- **90일 지표는 창 안으로 잘린다.** `distinct_authors_90d`(§2.1)와 `change_exposure_90d`(§5.5)는 `[HEAD_TIME − 90d, HEAD_TIME] ∩ W`에서 계산한다. 커밋 수 상한이 90일보다 짧은 구간을 남기면(`bound_hit: count`) 두 지표는 잘린 구간만 본다 — 창 밖 커밋이 값을 움직이면 같은 `analysis_input_id`가 다른 값을 내고 D87이 깨진다. 시간 상한은 90일을 자를 수 없다(90일 ⊂ 12개월) — 절단은 `count`에서만 일어난다.
