...
**배터리 리포**(나이·크기로 선정; AI 사용 여부로 고르지 않는다 — 선택 효과): 노년(15년+) Apache Commons Lang·Spring Framework·Netty·Elasticsearch(P5·P11·옛 커밋 빌드의 혹독한 시험대), 중년(5–12년) Micronaut·Ghidra·Halo(P2·머지 정책 비교), 청년(≤3년) Conductor(GitHub 생성일·Java·Gradle·500+ 커밋), Kotlin 혼합 1개(pkl), 선형 이력 리포 1개(Commons Lang — 대조는 Spring Framework·Ghidra), 깊은 기업형 패키지 합성 리포 1개(P8), 악성 픽스처 리포 1개(P9). jQRadar 자기 리포는 S1 이후 **내부자 데이터**로 병기하되 배터리를 대체하지 않는다(§9 자기 적용).
...
### G0 체크리스트
동결 승인 조건 — 전부 충족:
1. **§2.1–2.9** 계약이 리뷰어 2명의 **교차 검토**(계약 간 충돌 점검 포함)를 통과. 리뷰어 둘 중 최소 한 명은 이 리포에 커밋한 적 없는 외부 사람이고, 이 문서·픽스처를 만든 도구는 리뷰어가 될 수 없다(D84). **§6.3–6.6은 여기서 빠진다** — 그 계약들의 값 층 픽스처가 G3(`contract/validation·security`)에 오므로 지금 검토하면 문장만 읽게 되고, "검토했다"가 "문장만 읽었다"를 감춘다. G3 입장 조건으로 옮긴다(§9 일정).
