...
| `history/` | 앵커 고정(같은 HEAD 다른 날짜), 머지 제외, rename(tie-break·`rename_ambiguous`), 상한 두 종류, coarse, **shallow clone → `age_unknown`**, 음수 나이 → `invalid_metadata`, **`scan`·`change` 경로에서 blame 호출 시 실패**(부재 검사이므로 `must_be_caught` + `must_not_be_caught`) | G0 |
