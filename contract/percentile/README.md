...
3a. **분모의 이름은 `n_ranked`**(D116·D122) — 성분·렌즈 공통이며 그 값이 non-null인 파일 수다. 모집단 크기와 다를 때만 `n_population`을 병기한다. 인쇄된 분모로 pct를 재계산할 수 없으면 그 pct는 의견이다(B.4).
4. **분위(quantile)** — type-7 선형 보간(Hyndman–Fan). `h = 1 + q·(N − 1)`, `Q(q) = x_⌊h⌋ + (h − ⌊h⌋)·(x_⌊h⌋₊₁ − x_⌊h⌋)`. **모든 분위에 대한 단일 선언**이다(D114): `median = Q(0.5)`, `IQR = Q(0.75) − Q(0.25)`도 같은 방법이며, §2.5 표시 규칙과 §3.7 confidence가 둘을 쓰므로 방법이 갈리면 confidence가 갈린다.
...
## 산술 — 부동소수점을 쓰지 않는다 (§2.5 산술 계약, D115·D121)

`compute.py`는 전부 `fractions.Fraction`(정확 유리수)으로 계산하고, 직렬화는 공용 [`contract/tools/exact.py`](../tools/exact.py)가 한다 — 직렬화가 계산기마다 갈리면 같은 값이 다른 바이트가 되고 "두 머신 바이트 동일"(§2.9)이 계산기마다 다른 뜻이 된다. 계약이 그렇게 정한다 — 순위·분위·임계 비교는 **반올림 전 정확값**으로 판정하고, 직렬화만 백분위 4자리·렌즈 1자리 HALF_EVEN이다. `expected.json`에 실리는 것은 **반올림 전 값**이다(임계 비교의 대상이 그것이므로). IEEE-754 double로 §2.5의 픽스처 예를 그대로 평가하면 **18.1이 나오지 않는다**:
...
| `n_ranked ≠ n_population` | 렌즈 케이스가 26/21로 이미 덮는다 | [`lens-percentile-zero-included/`](lens-percentile-zero-included/) |
| `median`·`IQR`도 type-7 | P90 케이스가 같은 배열에서 median 5.5·IQR 4.5를 함께 낸다 | [`p90-type7/`](p90-type7/) |
| 언어 분리 | `cx` 백분위가 java/kotlin 모집단에서 따로 계산됨(D12) | **미작성** |
| 0 팽창 | 0이 다수인 성분에서 동점 평균 순위가 0에 주는 pct | **미작성** |

미작성 둘은 G0 동결 전에 필요하다(§9 G0 #2·#3). 0 팽창은 지금 `lens-percentile-zero-included/`(0 열넷)와 `iqr-zero-low-confidence/`(0 151개) 안에서 부수적으로 돌지만 자기 케이스가 없다.
