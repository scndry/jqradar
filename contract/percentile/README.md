...
`compute.py`는 전부 `fractions.Fraction`(정확 유리수)으로 계산한다. 계약이 그렇게 정한다 — 순위·분위·임계 비교는 **반올림 전 정확값**으로 판정하고, 직렬화만 백분위 4자리·렌즈 1자리 HALF_EVEN이다. `expected.json`에 실리는 것은 **반올림 전 값**이다(임계 비교의 대상이 그것이므로). IEEE-754 double로 §2.5의 픽스처 예를 그대로 평가하면 **18.1이 나오지 않는다**:
