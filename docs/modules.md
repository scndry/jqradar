...
| 4 | 측정·순위 경로에서 `double`·`float` 금지 — 필드·**파라미터**·반환 타입(메서드 **와 생성자**)·박싱 의존 | §2.5·D115·D121 | 반례만 |
...
**본다**: 필드 타입 · **모든 code unit의 파라미터와 반환 타입**(메서드 *와 생성자*) · 본문의 박싱 타입 의존. 배열은 성분 타입으로 판정한다.

파라미터를 보는 것이 핵심이다. 첫 판은 필드·반환 타입·박싱 셋만 봐서 `BigDecimal hotspot(double, double)`을 **통과시켰다** — 경계로 들어오는 double이 측정 경로에서 가장 흔한 경로이고, 값이 이미 double이면 안에서 `BigDecimal`로 감싸도 정밀도는 돌아오지 않는다. 생성자도 `methods()`에 잡히지 않아 `codeUnits()`로 바꿨다. 반례 `TakesDoubleAtTheBoundary`와 회귀 잠금 테스트가 그 자리를 지킨다.

**못 본다**:

- **지역 변수와 중간 식.** 바이트코드의 `LocalVariableTable`은 선택적이라 신뢰할 수 없다. 규칙은 **API 표면**을 지키고, 값 수준의 회귀는 `contract/reproducibility/`의 "두 머신 바이트 동일"이 잡는다.
- **`Math.sqrt` 같은 `double` 반환 JDK 호출.** D115가 "√는 표시값에만 `BigDecimal.sqrt`"라고 정한 자리라 규칙으로 좁힐 여지가 있으나, `Math`는 정수 연산도 담고 있어 통째로 막으면 과하다. 계약이 목록을 정하기 전까지 열어 둔다 — **PRD 공백으로 표시한다.**
