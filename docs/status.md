...
```
    G0 #1 열린 발견 — 기록에서 센 수 (D140·D151·D152)
      ...
      범위 안 `dedupe_key`  총 30
        닫힘  14 — D111, D126, D146, D147, D148, D149, D150, D154, D155, D156, D157, D158, D159, D160, D161, D162
        열림  16
      범위 밖 (G3 목록으로) 2
...
**총과 열린이 처음으로 갈렸다.** 닫힘 표시가 기록에 붙으면서 `열림 25 < 총 30`이 **계산**에서 나왔고, v3.9.0 뒤 `열림 19`, v3.9.1 뒤 `열림 16`이 됐다. 닫힌 열넷은 S1(D146–D150)·R2-3(D111)·N4(D148)·N7(D111·D126)·N8(D148)과 v3.9.0의 여섯 S5(D154)·S6(D159)·S13(D156)·R4-2(D157)·N3(D158)·O-3(D155)과 v3.9.1의 셋 S2(D160)·S9(D161)·R4-6(D162)이고, 표마다 **근거 열**이 왜 그 D가 그 주장을 닫는지 적는다 — 계산기는 그 열을 읽지 않는다(표시의 유무만 본다). `prd.md`를 읽어 추측하지 않으므로 표시 없는 25는 열린 것이다.
