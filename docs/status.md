...
**있는 것**: `prd.md` v3.9.0(D1–D159, §0-29 보정 2026-09-22 포함). `contract/` 케이스 **100종** — percentile 6 · **schema 60** · cpd 5 · history 15 · graph 8 · **reproducibility 6**. `schemas/` 7종. Gradle 골격과 자체 ArchUnit 규칙 넷 + 반례(11 tests). CI **5잡**. `docs/preregistration/p1a.md`, `docs/battery.md`(실측).
...
```
    G0 #1 열린 발견 — 기록에서 센 수 (D140·D151·D152)
      ...
      범위 안 `dedupe_key`  총 30
        닫힘  5 — D111, D126, D146, D147, D148, D149, D150
        열림  25
      범위 밖 (G3 목록으로) 2
...
**총과 열린이 처음으로 갈렸다.** 닫힘 표시가 기록에 붙으면서 `열림 25 < 총 30`이 **계산**에서 나온다. 닫힌 다섯은 S1(D146–D150)·R2-3(D111)·N4(D148)·N7(D111·D126)·N8(D148)이고, 표마다 **근거 열**이 왜 그 D가 그 주장을 닫는지 적는다 — 계산기는 그 열을 읽지 않는다(표시의 유무만 본다). `prd.md`를 읽어 추측하지 않으므로 표시 없는 25는 열린 것이다.
