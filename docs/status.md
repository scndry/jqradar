    ...
    G0 #1 열린 발견 — 기록에서 센 수 (D140·D151·D152)
      ...
      **닫힘 표시는 기록에서만 읽는다** — `닫힘 — D<n>`이 적힌 key만 닫힌 것으로 센다.
      ...
      보고 — 세지 않았거나 어긋난 것:
        ...
        [출처 없음] 2026-09-14-종합-2차-다중모델-패널.md: N4 — 병합한 개별이 없다. 세지 않는다
...
**총과 열린을 가른다.** 위 "열림 27"은 **닫힘 표시가 아직 기록에 하나도 없어서** 총과 같다 — 발견 1이 D146–D149로, §2.7의 D137 근거가 D150으로 닫힌 것은 `prd.md`에 있지만 **기록에 `닫힘 — D<n>`으로 적히지 않았다.** 계산기는 `prd.md`를 읽어 추측하지 않는다(그러면 그 수가 다시 사람 보고가 된다). 닫힘 표시를 기록에 넣는 것이 다음 PR이고, **그때 비로소 "열린 발견"이 계산된 수가 된다.**
